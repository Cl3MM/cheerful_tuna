#encoding: utf-8
#
class CollectionPointsPdf < Prawn::Document
  def initialize(collection_point, locales = "en", specimen = false)
    I18n.locale = locales
    @specimen = specimen
    ...
    if collection_point
      ...
      #filename = "#{Rails.root}/app/assets/fonts/collection_pointship_collection_point_certificate.pdf"
      super(page_size: "A4",
            bottom_margin: 0,
            top_margin: 0,
            right_margin: 0,
            left_margin: 0,
            info: {
              #Title:  I18n.t("collection_point_certificate.title", name: "#{@collection_point.name.html_safe}"),
              Author: I18n.t("collection_point_certificate.author"),
              Keywords: "CERES collection_pointship Certificate #{@collection_point.name.html_safe} http://www.ceres-recycle.org",
              ...
              CertifId: "#{@collection_point.id.to_s.html_safe}",
              #Subject: "This documents certify that #{@collection_point.name.html_safe}, #{@collection_point.postal_code.to_s.html_safe} \
              ##{@collection_point.city.html_safe.capitalize}, #{@collection_point.country.html_safe.upcase} is a collection_point of CERES \
#for the period #{@collection_point.start_date.strftime('%B %d, %Y')} \
#to #{(@collection_point.start_date + 1.year).strftime('%B %d, %Y')}",
            }
           )
  ...
  def body
    opts = [
      ...
      },
      {
        ...
        text: I18n.t("collection_point_certificate.president"),
        options: {size: 11, :leading => 5, shift: 10, align: :center }
      }
    ]
  ...
  def display_name
    ...
    bounding_box([115,590], width: 470, height: 90) do
      text @collection_point.name.html_safe, size: 56, align: :center, valign: :center, overflow: :shrink_to_fit
    end
    font @fonts[:MPR]
    bounding_box([115,490], width: 470, height: 100) do
      text "#{@collection_point.address.html_safe}\n#{@collection_point.postal_code.html_safe}, #{@collection_point.city.html_safe.titleize}\n#{@collection_point.country.html_safe.capitalize}",
      size: 28, align: :center, valign: :center, overflow: :shrink_to_fit, leading: 5
    end

    bounding_box([115,270], width: 470, height: 30) do
      #text I18n.t("collection_point_certificate.expiry", date: "#{@collection_point.end_date_to_human}"), size: 16, align: :center,
      text I18n.t("collection_point_certificate.expiry", date: "31 Décembre 2013"), size: 16, align: :center,
            valign: :center, overflow: :shrink_to_fit, leading: 5
    end

    tmp_qr_code = "#{Rails.root}/tmp/#{@collection_point.object_id}.png"
    url = "http://www.ceres-recycle.org"
    qr_code = %x(qrencode -s 2 -o #{tmp_qr_code} #{url})
    image tmp_qr_code, at: [518,86]
    FileUtils.rm(tmp_qr_code)
  end
