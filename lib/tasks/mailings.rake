...
namespace :mail do
  ...
  task :italians => :environment do
    ...
    italian_emails  = Contact.where("country = ?", ["Italy"]).joins(:emails).select('emails.address').to_set
    members_emails  = Contact.tagged_with("member").joins(:emails).select('emails.address').to_set
